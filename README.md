...
StartRes = es_core_store:start(StoreContext),
...
Res1 = es_core_mgr_aggregate:dispatch(
    BankMgr,
...
Res2 = es_core_mgr_aggregate:dispatch(
    BankMgr,
...
Res3 = es_core_mgr_aggregate:dispatch(
    BankMgr,
...
```erlang
es_core_aggregate:start_link(Module, Store, Id, #{timeout => 10000}).
```
...
2. If the aggregate type matches the manager's configured `Aggregate` module:
   ...
   - If none exists, it spawns a new `es_core_aggregate` process using the provided Aggregate, Store, and stream ID, then monitors it.
   - The command is forwarded to the aggregate process via `es_core_aggregate:dispatch/2`.
...
apps/
├── es_contract
│   ├── include/es_contract.hrl                     % Shared types and records
│   └── src                                         % Public behaviours (the contract)
│       ├── es_contract.app.src
│       ├── es_aggregate_behaviour.erl
│       ├── es_event_store_behaviour.erl
│       └── es_snapshot_store_behaviour.erl
├── es_core
│   ├── src                                         % Core processes built on the contract
│   │   ├── es_core.app.src
│   │   ├── es_core_aggregate.erl
│   │   ├── es_core_mgr_aggregate.erl
│   │   ├── es_core_mgr_behaviour.erl
│   │   └── es_core_store.erl
│   └── test                                        % Aggregate + store suites
├── es_store_ets
│   ├── src/es_store_ets.erl            % ETS-backed store implementation
│   └── test                                        % ETS-focused tests (planned)
└── es_store_mnesia
    ├── src/es_store_mnesia.erl         % Mnesia-backed store implementation
