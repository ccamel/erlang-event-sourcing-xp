...
-define(ES_LIB, event_sourcing_store_mnesia).
...
retrieve_events(StreamId, Options) ->
    case ?ES_LIB:retrieve_and_fold_events(StreamId,
                                          Options,
                                          fun(Payload, Acc) -> [Payload | Acc] end,
                                          [])
    of
        {ok, ReversedEvents} ->
...
create_table_once_test() ->
    ...
    ?assertMatch({ok, _}, ?ES_LIB:start()),

    ?assertNotEqual(undefined,
                    mnesia:table_info(
                        ?ES_LIB:table_name(events), all)),
    ?assertEqual(true, is_table_empty(?ES_LIB:table_name(events))),
...
persist_single_event_test() ->
    ...
    {ok, EventId} = ?ES_LIB:persist_event(StreamId, Version, Payload),
    ?assertEqual(1, table_count(?ES_LIB:table_name(events))),
    ?assertMatch({ok,
                  [{event_record,
                    EventId,
                    "user_registered",
...
persist_2_streams_event_test() ->
    ...
    VersionA = 1,
    {ok, EventIdA} = ?ES_LIB:persist_event(StreamIdA, VersionA, PayloadA),
    ?assertEqual(1, table_count(?ES_LIB:table_name(events))),

    StreamIdB = "stream_B",
    PayloadB = {user_registered, "Jane Doe"},
    VersionB = 1,
    {ok, EventIdB} = ?ES_LIB:persist_event(StreamIdB, VersionB, PayloadB),
    ?assertEqual(2, table_count(?ES_LIB:table_name(events))),
...
fetch_streams_event_test() ->
    ...
    Version1 = 1,
    {ok, EventId1} = ?ES_LIB:persist_event(StreamId, Version1, Payload1),

    Payload2 = {user_updated, "John Doe"},
    Version2 = 2,
    {ok, EventId2} = ?ES_LIB:persist_event(StreamId, Version2, Payload2),

    Payload3 = {user_deleted},
    Version3 = 3,
    {ok, EventId3} = ?ES_LIB:persist_event(StreamId, Version3, Payload3),
