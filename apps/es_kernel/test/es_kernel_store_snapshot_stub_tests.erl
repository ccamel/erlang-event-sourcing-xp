-module(es_kernel_store_snapshot_stub_tests).
...
setup() ->
    es_kernel_store_snapshot_stub:stop(),
    ok.
...
concurrent_start_operations() ->
    ...
    Pids = [
        spawn(fun() ->
            Result = es_kernel_store_snapshot_stub:start(),
    ...
    ?assertEqual(ok, es_kernel_store_snapshot_stub:stop()).

concurrent_stop_operations() ->
    ?assertMatch(ok, es_kernel_store_snapshot_stub:start()),
    ...
    Pids = [
        spawn(fun() ->
            Result = es_kernel_store_snapshot_stub:stop(),
